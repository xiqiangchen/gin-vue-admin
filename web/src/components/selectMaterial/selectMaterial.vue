<template>
  <div
    ...
    class="update-image"
    :style="{
      'background-image': `url(${getUrl(modelValue.url)})`,
      'position': 'relative',
    }"
  >
    <el-icon
      v-if="isVideoExt(modelValue.url || '')"
      :size="32"
    ...
    <video
      ...
      @click="openChooseImg"
    >
      <source :src="getUrl(modelValue.url) + '#t=1'">
    </video>
    <span
      v-if="modelValue.url"
      class="update"
  ...
  >
    <div
      ...
      class="update-image"
      :style="{
        'background-image': `url(${getUrl(item.url)})`,
        'position': 'relative',
      }"
    >
      <el-icon
...
const chooseImg = (id, url) => {
  ...
  if (props.multiple) {
    multipleValue.value.push({url:url, id:id})
    emits('update:modelValue', multipleValue.value)
  } else {
    emits('update:modelValue', {url:url, id:id})
  }
  drawer.value = false
}
const openChooseImg = async() => {
  if (props.modelValue && !props.multiple) {
    emits('update:modelValue', {id:0, url:''})
    return
  }
  await getMaterials()
  drawer.value = true
