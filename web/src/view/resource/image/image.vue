    ...
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" :style="{ width: '185px' }" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="图片格式" prop="format">
         <el-input v-model="searchInfo.format" :style="{ width: '80px' }" placeholder="图片格式" />
        </el-form-item>
        <el-form-item label="宽" prop="width">
             <el-input v-model.number="searchInfo.width" :style="{ width: '80px' }" placeholder="宽" />
        </el-form-item>
        <el-form-item label="高" prop="height">
         <el-input v-model="searchInfo.height" placeholder="高" :style="{ width: '80px' }" />
        </el-form-item>
        <el-form-item label="备注" prop="comment">
         <el-input v-model="searchInfo.comment" placeholder="备注" :style="{ width: '80px' }" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="日期" width="180">
          ...
          <el-table-column label="链接" width="200">
              <template #default="scope">
                <el-image style="width: 100px; height: 100px" :src="getUrl(scope.row.url)" fit="cover"/>
              </template>
          </el-table-column>
        <el-table-column align="left" label="图片格式" prop="format" width="120" />
        ...
        <el-table-column align="left" label="操作" min-width="120">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateImageFunc(scope.row)">变更</el-button>
      ...
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="图片链接:"  prop="url" >
              <el-input v-model="formData.url" :clearable="true"  placeholder="请输入图片格式" />
            </el-form-item>
            <el-form-item label="图片格式:"  prop="format" >
              <el-input v-model="formData.format" :clearable="true"  placeholder="请输入图片格式" />
            </el-form-item>
            <el-form-item label="宽:"  prop="width" >
...
watchEffect(() => {
  if (image_tmp_url.value !== '') {
    formData.value.url = image_tmp_url.value;
  }
});
...
const getTableData = async() => {
  const table = await getImageList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
// 更新行
const updateImageFunc = async(row) => {
    const res = await findImage({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteImageFunc = async (row) => {
